...
if (!isset($arParams["COLUMNS_LIST"]) || !is_array($arParams["COLUMNS_LIST"]) || count($arParams["COLUMNS_LIST"]) <= 0)
	$arParams["COLUMNS_LIST"] = array("NAME", "PRICE", "TYPE", "QUANTITY", "DELETE", "DELAY", "WEIGHT", "NUMBER");
...
if(!isset($arParams["PROPERTY_LIST"]) || !is_array($arParams["PROPERTY_LIST"])) {
    $arParams["PROPERTY_LIST"] = array();
}
...
while ($arItems = $dbBasketItems->GetNext())
{
    ...
    if( CModule::IncludeModule ('iblock')
        && 0 < $arItems["PRODUCT_ID"]
        && is_array($arParams["PROPERTY_LIST"])
        && count($arParams["PROPERTY_LIST"]) > 0
    ) /*{
        $arFilter=array (
            "ID" => $arItems["PRODUCT_ID"],
            "ACTIVE" => "Y"
        );
        $arSelectedFields = array("ID", "IBLOCK_ID");
        $db_props = CIBlockElement::GetList(Array("ID" => "DESC","enum_sort" => "DESC"), $arFilter, false, false, $arSelectedFields);
        if ($ob = $db_props->Fetch()) {
            $infoBlockID = $ob['IBLOCK_ID'];

            unset($db_props);

            $db_props = CIBlockElement::GetProperty($infoBlockID, $arItems["PRODUCT_ID"], array("sort" => "asc"), array("ACTIVE" => "Y"));
        ...
        foreach ($arParams["PROPERTY_LIST"] as $property) {
            $arSelectedFields[] = "PROPERTY_" . $property;
        }

        $db_props = CIBlockElement::GetList(Array("ID" => "DESC", "enum_sort" => "DESC"), $arFilter, false, false, $arSelectedFields);

        if ($ob = $db_props->GetNextElement()) {
            $arItems['PROPERTY_LIST'] = $ob->GetProperties();
        }
    }
    //
